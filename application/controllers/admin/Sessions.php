...
class Sessions extends Home_Controller {
    ...
    {   
        ...
        {   
            ...
            $notify = array(
                ...
                'text' => trans('you-have-booked-a-session-').' '.'"'.' '.$session->name.' '.'"'.' '.trans('of') .' '. $mentor->name.' ' .trans('just-now') ,
                'noti_type' => 2,
    ...
    public function status_update($status, $id) 
    {
        ...
        if($status == 1){
            $text = $mentor_name .'  '. trans('approved-your-booked-session-') .' '.'"'.$session_name.' ';
            $status = trans('approved');
            $status_email_text = trans('approved');
        }
        if($status == 2){
            $text = $mentor_name .trans('rejected-your-booked-session-') .'"'.$session_name.'"';
            $status = trans('reject');
            $status_email_text = trans('rejected');
        }
        if($status == 3){
            $text = trans('the-session-you-have-booked-') .'"'.$session_name.'" '.trans('has-been-completed');
            $status = trans('complete');
            $status_email_text = trans('completed');
        }
        ...
        $msg = $this->load->view('email_template/common', $edata, true);
        $response = $this->email_model->send_email($mentee_email, $subject, $msg);
        ...
        if(empty($coupon)){
            ...
            if (date('Y-m-d') >= $coupon->start_date && date('Y-m-d') <= $coupon->end_date) {
                ...
                if ($coupon->usages_limit == 0) {
                    echo json_encode(array('st' => 0, 'msg' => '<i class="fas fa-times-circle"></i> '.trans('this-coupon-code-has-no-limit'))); exit();
                }
    ...
    {   
        ...
        {   
            ...
            $notify = array(
                ...
                'content_id' => 0,
                'text' => $mentee_name. ' '. trans('gives') .' ' .$rating->rating . ' ' . trans('star-rating-on-your-session-') .'"'.$session_name.'"',
                'noti_type' => 5,
                'noti_time' => my_date_now()
            );
            $notify = $this->security->xss_clean($notify);
    ...
    public function add_meeting($id){
        ...
        $agenda = trans('zoom-meeting-session-with-').$mentor->name;
