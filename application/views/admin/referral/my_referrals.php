                    ...
                    <tbody>
                      <?php $i=1; foreach ($referrals as $referral): ?>
                        <tr id="row_<?php echo html_escape($referral->id); ?>">
                          ...
                          <td>
                            <?php if(settings()->curr_locate == 0){echo html_escape(settings()->currency_symbol);} ?>
                            <?php echo number_format($referral->amount, settings()->num_format) ?>
                            <?php if(settings()->curr_locate == 1){echo html_escape(settings()->currency_symbol);} ?>  
                          </td>
                          <td><?php echo html_escape($referral->commision) ?></td>
                          <td>
                            <?php if(settings()->curr_locate == 0){echo html_escape(settings()->currency_symbol);} ?>
                            <?php echo html_escape($referral->commision_amount, settings()->num_format) ?>
                            <?php if(settings()->curr_locate == 1){echo html_escape(settings()->currency_symbol);} ?>  
                          </td> 
