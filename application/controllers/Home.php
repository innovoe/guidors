...
class Home extends Home_Controller {
    ...
    {   
        ...
        $data['count_categories'] = is_array($data['categories']) ? count($data['categories']) : 0;
    ...
    public function send_message_mentor() 
    {
        ...
        if ($_POST) {
            ...
            $notify = array(
                ...
                'text' => $sender->name .trans('sent-you-a-message'),
    ...
    {  
        ...
        $notify = array(
            ...
            'text' => $mentee_name.' '.trans('just-booked-your-session') .'"'.$session_name.'"',
            'noti_type' => 2,
            'noti_time' => my_date_now()
        );
        ...
        $notify = array(
            ...
            'action_id' =>$single_booking->mentee_id,
            'content_id' =>$single_booking->booking_number,
            'text' => trans('you-have-booked-a-session') .'"'.$session_name.'" '.' '.trans('of').' '. $mentor->name.' '. trans('just-now'),
            'noti_type' => 2,
            'noti_time' => my_date_now()
        );
        $notify = $this->security->xss_clean($notify);
