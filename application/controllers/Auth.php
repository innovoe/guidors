...
class Auth extends Home_Controller 
{
    ...
    public function log()
    {
        ...
        if($_POST){ 
            ...
            if ($user->role == 'user') {
                ...
                if (settings()->enable_regtration_approval == 1 && $user->status == 0) {
                    echo json_encode(array('st'=>2));
                    exit();
                }
    ...
    public function register_user()
    {
        ...
        if($_POST){
            ...
            if (settings()->enable_regtration_approval == 1) {
                $status = 0;
            }else{
            ...
            if($this->form_validation->run() == FALSE){
                ...
                if ($email){
                    ...
                    if (!$this->recaptcha_verify_request()) {
                        ...
                        if (settings()->enable_regtration_approval ==1) {
                            //send email to user
                            $reg_subject = get_email_by_slug('registrant-confirmation')->subject;
                            $reg_body = get_email_by_slug('registrant-confirmation')->body;
                            $variables_data = [
                                'user_name'  =>$user->name
                            ]; 

                            $reg_msg = preg_replace_callback('/{{(.*?)}}/', function ($matches) use ($variables_data) {
                                $key = trim($matches[1]);
                                return isset($variables_data[$key]) ? $variables_data[$key] : $matches[0]; 
                            }, $reg_body);

                            $edata = array();
                            $edata['subject'] = $reg_subject;
                            $edata['msg'] = $reg_msg;

                            $reg_msg = $this->load->view('email_template/common', $edata, true);
                            $response = $this->email_model->send_email($this->input->post('email'), $reg_subject, $reg_msg);


                            //send email to admin
                            $adminnew_subject = get_email_by_slug('admin-new-application-alert')->subject;
                            $adminnew_body = get_email_by_slug('admin-new-application-alert')->body;
                            $adminvariables_data = [
                                'user_name'  =>$user->name,
                            ...
                            $adminnew_msg = preg_replace_callback('/{{(.*?)}}/', function ($matches) use ($adminvariables_data) {
                                $key = trim($matches[1]);
                                return isset($adminvariables_data[$key]) ? $adminvariables_data[$key] : $matches[0]; 
                            }, $adminnew_body);

                            $edata = array();
                            $edata['subject'] = $adminnew_subject;
                            $edata['msg'] = $adminnew_msg;

                            $adminnew_msg = $this->load->view('email_template/common', $edata, true);
                            $response = $this->email_model->send_email(settings()->admin_email, $adminnew_subject, $adminnew_msg);
                        ...
                        if (settings()->enable_email_verify == 1) {
                            ...
                            $variables_data = [
                                'user_name'  =>$user->name,
                                'site_name' => lang_value()->site_name,
                                'verify_code' => $code,
                            ]; 
    ...
    public function send_notify_mail($id)
    {
        ...
        $subject = $session->name. trans('live-session-notify-mail');
        
        $msg = trans('hello').' '.$mentee->name.', <br> ' . trans('you-have-booked-an-session-with') . ' <b>' 
            . $mentor->name . '</b> ' . trans('which-will-start-at') . ' ' . my_date_show($booking->date) . ' ' 
            . $booking->time;
        $msg = trans('hello').'  ' . $mentee->name.', <br> '. trans('you-have-booked-an-session-with') .'<b>'.$mentor->name.'</b>'.' '. trans('which-will-start-at').my_date_show($booking->date).' '.$booking->time;
        //echo "<pre>"; print_r($msg); exit();

        $result = $this->email_model->send_email($mentee->email, $subject, $msg);
        if ($result == true) {
            $this->session->set_flashdata('msg', trans('notify-mail-send-successfully')); 
            redirect($_SERVER['HTTP_REFERER']);
        }else{ 
            $this->session->set_flashdata('error', trans('email-sending-failed-please-check-your-smtp-connections')); 
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
