...
class Settings extends Home_Controller {
    ...
    public function update(){
        ...
        if ($_POST) {
            ...
            if(!empty($this->input->post('enable_regtration_approval', true))){$enable_regtration_approval = $this->input->post('enable_regtration_approval', true);}
            else{$enable_regtration_approval = 0;}
            ...
            $data = array(
                ...
                'enable_mentor_auto_approve' => 1,
                'enable_regtration_approval' => $enable_regtration_approval,
    ...
    public function add_mentor_settings_lang()
    {
        $languages = $this->admin_model->select('language');

        foreach ($languages as $language) {
            $settings_lang =  $this->admin_model->get_by_langid_mentor($language->id);
            if ($settings_lang == 0) {
                $data=array(
                    'lang_id' => $language->id
                );
                $this->admin_model->insert($data, 'mentors_settings_extra');
            }
        }
    }
    ...
    public function load_category_skill() 
    {
        ...
        if ($posted_user_id === null || $posted_user_id === false || $posted_user_id === '') {
            $data['user_skills'] = $this->admin_model->get_skill_by_user(user()->id);
        } else {
            $uid = (int) $posted_user_id;
            $data['user_skills'] = $uid > 0 ? $this->admin_model->get_skill_by_user($uid) : array();
        }
