  ...
  'use strict';
    ...
    $(document).ready(function(){
      ...
      $(document).on('change', ".skill_category", function() {
          var Id = $(this).val();
          var payload = { data: 'value', 'csrf_test_name': csrf_token, 'id': Id };
          if ($('#mentor_skill_user_id').length) {
              payload.user_id = $('#mentor_skill_user_id').val();
          }
    ...
    $(document).on('click', ".session_price_type", function() {
        var val = $(this).val();

        if (val == 1) {
           $('.minimum_price_limit').show();
        }else{
          $('.minimum_price_limit').hide();
          $('.session_price').val(0);
        }
