...
class Category extends Home_Controller {
    ...
    {   
        ...
        {   
            ...
            $check = $this->admin_model->check_keyword(str_slug($this->input->post('name', true)));
            if ($check == 1) {

            } else {

                $data=array(
                    'label' => $this->input->post('name', true),
                    'keyword' => character_limiter(str_slug($this->input->post('name', true)), 2),
                    'english' => $this->input->post('name', true),
                    'type' => 'admin'
                );
                $data = $this->security->xss_clean($data);
    ...
    public function add_category_language()
    {   
        $categories = $this->admin_model->get_admin_categories();


        foreach ($categories as $category) {
            $check = $this->admin_model->check_keyword(str_slug($category->name));
            if ($check == 1) {
                ...
                $data=array(
                    'label' => $category->name,
                    'keyword' => character_limiter(str_slug($category->name), 2),
